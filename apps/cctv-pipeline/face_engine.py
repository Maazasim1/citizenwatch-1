...
EMBEDDING_MATCH_THRESHOLD = 0.78
EMBEDDING_STRONG_MATCH_THRESHOLD = 0.84
...
def train_model():
    ...
    """
    global _lbph_model, _lbph_names, _model_trained, _subject_embedding_gallery
...
def recognize_faces_in_frame(frame):
    ...
    for face_coord in face_coords:
        ...
        # Cross-reference against all stored images per subject via embeddings.
        emb_name = None
        emb_conf = 0.0
        emb_margin_ok = False
        query_embedding = _compute_embedding_from_face_crop(face_bgr)
        if query_embedding is not None and _subject_embedding_gallery:
            best_name = None
            best_sim = -1.0
            second_best_sim = -1.0
            for subject_name, emb_list in _subject_embedding_gallery.items():
                if not emb_list:
                    continue
                sim = max(cosine_similarity(query_embedding, e) for e in emb_list)
                if sim > best_sim:
                    ...
                    second_best_sim = sim
            if best_name is not None and best_sim >= EMBEDDING_MATCH_THRESHOLD:
                emb_name = best_name
        ...
        emb_acceptable = emb_name is not None and (
            emb_conf >= EMBEDDING_STRONG_MATCH_THRESHOLD or emb_margin_ok
        )
...
def register_criminal_samples(name: str, images_data: list, fir_number: str = "", append: bool = False):
    ...
    if len(images_data) > 0:
        ...
        cv2.imwrite(os.path.join(profile_dir, profile_name), images_data[0])
    ...
    return {
        ...
        "profile_image": f"mugshot_{safe_name}.jpg",
        "append_mode": append,
    }
...
def compute_face_embedding(image_path: str) -> np.ndarray | None:
    """
    Compute a feature vector for the largest face in the image.
    Pilot approach: resize face to 64x64, compute LBP histogram + color histogram.
    This gives a ~500-dim feature vector suitable for cosine similarity.
    """
    img = cv2.imread(image_path)
    if img is None:
        return None
    ...
    # Color histograms (H and S channels from HSV)
    ...
    # Structural features: edge orientation histogram
    ...
    embedding = np.concatenate([gray_hist, h_hist, s_hist, angle_hist])
    return embedding


def _compute_embedding_from_face_crop(face_bgr: np.ndarray) -> np.ndarray | None:
    """Compute embedding directly from a face crop (BGR)."""
...
def _build_subject_embedding_gallery() -> dict:
    """
    Build per-subject embedding gallery from all original sample images.
    ...
    for subject in sorted(os.listdir(originals_root)):
        ...
        for fname in sorted(os.listdir(subject_dir)):
            ...
            emb = compute_face_embedding(fpath)
            if emb is not None:
                subject_embs.append(emb)
        if subject_embs:
...
def add_criminal(name: str, fir_number: str, mugshot_path: str) -> dict | None:
    ...
    embedding = compute_face_embedding(mugshot_path)
    if embedding is None or img is None:
        return None

    # Build multi-view embeddings from simple pose/brightness augmentations.
    embeddings = [embedding]
    augmentations = []
    for angle in (-15, -8, 8, 15):
        h, w = img.shape[:2]
        M = cv2.getRotationMatrix2D((w / 2, h / 2), angle, 1.0)
        augmentations.append(cv2.warpAffine(img, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT))
    augmentations.append(cv2.flip(img, 1))
    for aug in augmentations:
        tmp_path = os.path.join(CRIMINAL_DB_DIR, f"_tmp_{uuid.uuid4().hex}.jpg")
        try:
            cv2.imwrite(tmp_path, aug)
            emb = compute_face_embedding(tmp_path)
            if emb is not None:
    ...
    db = _load_db()
    db["criminals"].append(record)
    _save_db(db)

    # Also create face_samples entry for LBPH
    person_dir = os.path.join(FACE_SAMPLES_DIR, name)
    ...
    return {"id": criminal_id, "name": name, "fir_number": fir_number}
...
def match_face(image_path: str, threshold: float = 0.65) -> list:
    """
    Match a face in the given image against all criminals in the database.
    Returns sorted list of matches above threshold.
    """
    query_embedding = compute_face_embedding(image_path)
    ...
    for criminal in db.get("criminals", []):
        raw_embeddings = criminal.get("embeddings")
        if isinstance(raw_embeddings, list) and len(raw_embeddings) > 0:
            stored_embeddings = [np.array(e, dtype=np.float32) for e in raw_embeddings]
        else:
            stored_embeddings = [np.array(criminal["embedding"], dtype=np.float32)]

        similarity = max(cosine_similarity(query_embedding, se) for se in stored_embeddings)
